...
ruta_file_logs = os.path.join("pages", "logs", file_name_log)
# Asegurar existencia de directorios antes de usarlos
os.makedirs(os.path.dirname(ruta_file_logs), exist_ok=True)
ruta_files = os.path.join("pages", "files")
os.makedirs(ruta_files, exist_ok=True)

# Ajustar algoritmos de clave (fallback para conexiones legacy)
from paramiko import transport
transport.Transport._preferred_kex = ( # type: ignore
    'ecdh-sha2-nistp256',
    'ecdh-sha2-nistp384',
    'ecdh-sha2-nistp521',
    'ssh-rsa',
...
# Logging básico
logging.basicConfig(level=logging.INFO, handlers=[logging.handlers.WatchedFileHandler(ruta_file_logs)], format='%(asctime)s - %(name)s - %(levelname)s - %(message)s') # type: ignore
...
st.set_page_config(page_title="SSH Info", page_icon="🛠️", layout="wide")
...
/* Cards */
.stButton>button, .stFileUploader>div, .stTextInput>div {border-radius: 12px;}

/* Sidebar */
[data-testid="stSidebar"] {
    background: linear-gradient(180deg, #0b3d91 0%, #0f5eb0 100%);
}

h1, h2, h3, h4, h5, h6 {color: #0a2256;}

</style>
""", unsafe_allow_html=True)

if not st.session_state.get("authenticated", False):
    st.warning("Debes iniciar sesión en la página principal para usar esta herramienta.")
    st.stop()

st.title("🔒 SSH Info / Extracción de Inventario")
st.subheader("Conéctate a tus equipos y obtén detalles de producto en CSV")
