...
RUTA_PLANTILLAS="pages/plantillas/PLANTILLA_EQUIPOA.xlsx"

if not st.session_state.get("authenticated", False):
    st.warning("Debes iniciar sesion en la pagina principal.")
    st.stop()

st.set_page_config(page_title="Protecciones ⚡")
...
def write_log(ip, modelo, puerto, estado):
    ...
    hoy = datetime.now().strftime("%Y-%m-%d")
    filename = f"pages/logs/log_pro_{hoy}.csv"
...
st.set_page_config(
    page_title="Comprobacion de equipos A ",
    page_icon="🔎",
    layout="wide"
)
...
st.header("Comprobacion de equipos A ⚡")
...
df_pro = pd.read_json(JSON_EQUIPOS)
protecciones = df_pro["equipoA"][0]
listado_equipos = []
...
with tab1:
    ...
    proteccion_selecionada=st.selectbox("Selecciona el modelo",listado_equipos,key="modelo_unico",index=None)
    if proteccion_selecionada:
        st.success(f"Modelo: {proteccion_selecionada}")
        puerto = df_pro["equipoA"][0][proteccion_selecionada]
    ...
    with col1:
        if st.button("Probar conexión"):
            if not st.session_state["ip_nueva"]:
                ...
                st.info(f"Probando conexión a {st.session_state['ip']}:{puerto} (modelo {proteccion_selecionada})")
            ...
            write_log(ip, proteccion_selecionada, puerto, estado)
...
with tab2:
    ...
    if opcion_modelo:
        ...
        puerto_multiples = df_pro["equipoA"][0][opcion_modelo]
    ...
    if btn_probar:
        ...
        if not lista_ips.strip():
            ...
            resultados = []
            st.info(f"Probando {len(ips)} equipos en el puerto {puerto_multiples} ({opcion_modelo})...")
            ...
            for ip in ips:
                ...
                st.session_state["log"].append(registro_automatico)
                write_log(ip, opcion_modelo, puerto_multiples, estado)
...
with tab3:
    ...
    st.download_button(
        label="📥 Descargar plantilla equipo A",
        data=plantilla_bytes,
        file_name="plantilla_equipoA.xlsx",
        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
    )

    st.write("📌**Importante: Mirar la pestaña leyenda para saber los modelos de las protecciones.**")
    ...
    if file:
        ...
        with st.spinner("Obteniendo resultado...", show_time=True):
            ...
            for idx, row in df.iterrows():
                ...
                if df_pro["equipoA"][0][modelo]:
                    puerto = df_pro["equipoA"][0][modelo]
                    # Aquí haces tu comprobación real
                    estado = "OK" if check_port(ip, puerto) else "NOK"

                    # IMPORTANTE: SIEMPRE MISMA ESTRUCTURA
                    results.append([ip, modelo, puerto, estado, HOY])

                else:
                    # Modelo NO existe -> registrar error para que no falle el DataFrame
                    results.append([ip, modelo, None, "MODELO_DESCONOCIDO", HOY])
        ...
        st.download_button(
            ...
            data=csv,
            file_name="resultado_comprobaciones_protecciones.csv",
            mime="text/csv"
        )


with tab4:
    st.write("**Modelo de equipo A**⬇️")
    df_prosd = pd.read_json(JSON_EQUIPOS)
    prodfs = json_normalize(df_prosd['equipoA'])
    st.dataframe(prodfs.T)

with tab5:
    st.header("⚙️ Configuración")
    st.write("Aquí podrás añadir opciones de configuración, logs, parámetros avanzados, etc.")
    st.write("📘 Log de comprobaciones equipo A")
    ...
    st.download_button(
        ...
        data=csv_log,
        file_name="log_comprobaciones_equipoA.csv",
        mime="text/csv"
    )
